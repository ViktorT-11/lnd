...
type MigrationStream struct {
	...
	// MakePostMigrationChecks is a function that returns a map of
	// code migrations that will be executed after the corresponding SQL
	// migration has been applied. The key of the map is the migration
	// version, and the value is the function to be executed after the
	// SQL migration.
...
func applyMigrations(fs fs.FS, driver database.Driver, path,
	...
	if migrate.IsPostStepCallbackVersion(currentDbVersion) {
		log.Infof("Previously failed post migration detected for "+
			"migration %v, will re-attempt the post migration "+
			"prior to any database migration(s)",
			migrate.SQLMigrationVersion(currentDbVersion))
	}

	log.Infof("Attempting to apply migration(s) and post migration(s)"+
