...
func (r *RemoteSigner) Validate() error {
	...
	if r.AllowInboundConnection {
		if len(r.RPCListeners) == 0 {
			return fmt.Errorf("remotesigner.rpclisten must be set when " +
				"allowinboundconnection is enabled")
		}
...
type inboundWatchOnlyCfg struct {
	...
	// RPCListeners is an optional set of dedicated gRPC listener addresses
	// that will serve only the SignCoordinatorStreams RPC for inbound remote
	// signer connections. If unset, the main RPC server will serve the RPC.
	RPCListeners []string `long:"rpclisten" description:"Optional dedicated RPC listen address(es) for inbound remote signer connections. If set, lnd starts a separate gRPC server that serves only the SignCoordinatorStreams RPC. If unset, the main RPC server serves the RPC instead."`
}
